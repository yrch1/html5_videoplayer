...
function init() {
  ...
  videoContent.addEventListener('ended',onVideoEnd)

   var autoplayPlayList = videoContent.getAttribute("autoplayPlayList");
    console.log("autoplayPlayList: " + autoplayPlayList);
...
function onVideoEnd() {
    ...
    if ((currentVid + 1) >= lnkNum) { 
        ...
        var playListLoop = video.getAttribute("playListLoop");
        console.log("playListLoop: " + playListLoop);
        if(playListLoop!=null){
            nextVid = 0;
            playVid(nextVid);
        }else{
            console.log("end de la playlist");
        }
    } else { 
...
function playVid(index) {
    console.log('Reproduciendo ' + currentVid );
